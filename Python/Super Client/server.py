        ...
        if msg == "/viewRequests":
            ...
            if username == groups[groupname].admin:
                ...
                if usernameToApprove in groups[groupname].joinRequests:
                    ...
                    if usernameToApprove in groups[groupname].waitClients:
                        groups[groupname].waitClients[usernameToApprove].send(
                            b"/accepted")
                        groups[groupname].connect(
                            usernameToApprove, groups[groupname].waitClients[usernameToApprove])
                        del groups[groupname].waitClients[usernameToApprove]
                    print("Member Approved:", usernameToApprove,
                          "| Group:", groupname)
            ...
            if username == groups[groupname].admin:
                ...
                if newAdminUsername in groups[groupname].allMembers:
                    ...
                    client.send(
                        b"Your adminship is now transferred to the specified user.")
                else:
            ...
            if username == groups[groupname].admin:
                ...
                if usernameToKick in groups[groupname].allMembers:
                    ...
                    if usernameToKick in groups[groupname].onlineMembers:
                        groups[groupname].clients[usernameToKick].send(
                            b"/kicked")
                        groups[groupname].onlineMembers.remove(usernameToKick)
                        del groups[groupname].clients[usernameToKick]
                    print("User Removed:", usernameToKick,
                          "| Group:", groupname)
                    client.send(
                        b"The specified user is removed from the group.")
                else:
                    client.send(b"The user is not a member of this group.")
            else:
                client.send(b"You're not an admin.")
            ...
            while remaining:
                ...
                f.write(data)
            f.close()
            print("File received:", filename, "| User:",
            ...
            client.send(
                bytes(filename+" successfully sent to all online group members.", "utf-8"))
...
def handshake(client):
    ...
    if groupname in groups:
        if username in groups[groupname].allMembers:
            ...
            groups[groupname].sendMessage(
                username+" has requested to join the group.", "PyconChat")
            client.send(b"/wait")
            print("Join Request:", username, "| Group:", groupname)
        threading.Thread(target=pyconChat, args=(
            client, username, groupname,)).start()
    else:
